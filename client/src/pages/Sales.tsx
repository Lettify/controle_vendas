  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-slate-100">
      <main className="max-w-7xl mx-auto px-3 py-4 sm:px-4 sm:py-6 md:px-6 md:py-8 lg:px-8 lg:py-12 space-y-6 sm:space-y-8 lg:space-y-10">
        <SensitiveSection>
          <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-teal-600 via-emerald-500 to-cyan-500 text-white shadow-xl sm:rounded-3xl">
            <div className="absolute inset-0">
              <div className="absolute -top-24 -right-24 h-64 w-64 rounded-full bg-white/10 blur-3xl" />
              <div className="absolute bottom-0 left-0 h-48 w-48 translate-y-1/3 rounded-full bg-emerald-300/20 blur-3xl" />
            </div>

            <div className="relative flex flex-col gap-6 px-6 py-8 lg:flex-row lg:items-center lg:justify-between lg:px-10 lg:py-10">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center gap-2 rounded-full bg-white/20 px-3 py-1 text-xs font-bold text-white backdrop-blur-md">
                    <span className="h-1.5 w-1.5 rounded-full bg-lime-300 animate-pulse" />
                    {todayLabel}
                  </span>
                  <SensitiveSectionToggleButton showLabel={false} className="h-6 w-6 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-colors border-none bg-transparent shadow-none p-0 justify-center" />
                </div>
                <h1 className="text-3xl font-black tracking-tight text-white sm:text-4xl">
                  Nova Venda
                </h1>
                <p className="text-sm font-medium text-emerald-100/90 max-w-md">
                  Preencha os dados abaixo para registrar uma venda no sistema.
                </p>
              </div>

              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="flex items-center gap-4 rounded-2xl border border-white/20 bg-white/10 px-6 py-4 backdrop-blur-md shadow-lg">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-emerald-400 text-emerald-950 shadow-inner shadow-black/10">
                    <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wider text-emerald-100/80">Total em {selectedDate === getLocalDateString() ? "Hoje" : "Vendas"}</p>
                    <div className="flex items-baseline gap-2">
                      <SensitiveValue className="text-2xl font-black text-white tracking-tight">
                        {formatCurrency(sales.reduce((acc, sale) => acc + Number(sale.amount), 0))}
                      </SensitiveValue>
              ...
              <div className="rounded-2xl border border-emerald-100/70 bg-white/85 shadow-lg backdrop-blur-sm sm:rounded-3xl">
                <div className="flex items-center justify-between px-4 pt-4 sm:px-6 sm:pt-6">
                  <div>
                    ...
                    <h2 className="mt-0.5 text-base font-semibold text-slate-900 sm:mt-1 sm:text-lg">Escolha o dia</h2>
                  </div>
                  {isTodaySelected && (
                ...
                <div className="px-4 pb-5 pt-4 sm:px-6 sm:pb-6 sm:pt-5">
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="relative">
                          <Input
                            ...
                            className="w-full rounded-2xl border-2 border-emerald-100 bg-gradient-to-br from-white to-emerald-50 py-4 text-center text-4xl font-black leading-none text-emerald-700 shadow-sm transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
                            placeholder="00"
                          />
                          <span className="pointer-events-none absolute left-1/2 top-full mt-2 -translate-x-1/2 text-xs font-semibold uppercase tracking-widest text-emerald-500/60">
                            Dia
                          </span>
                        </div>
                        <button
                          ...
                          className={`flex h-full w-full items-center justify-center rounded-2xl border-2 px-3 text-sm font-semibold transition ${showCalendar
                      ...
                      {showCalendar && (
                        <div className="absolute top-full right-0 z-10 mt-3 w-full rounded-2xl border-2 border-emerald-100 bg-white p-4 shadow-2xl lg:w-80">
                          ...
                          <div className="mt-1 grid grid-cols-7 gap-1">
                            {calendarDays.map((calendarDay, index) => (
                              <button
                                ...
                                className={`aspect-square rounded-xl text-sm font-semibold transition ${!calendarDay
                                  ? "invisible"
                    ...
                    {/* Data Selecionada */}
                    <div className="rounded-xl border border-emerald-100 bg-gradient-to-br from-emerald-50/50 to-teal-50/30 px-4 py-3 text-center">
                      <p className="text-sm font-medium capitalize text-emerald-900">
                        {selectedDateLabel}
                      </p>
                    </div>
                  </div>
